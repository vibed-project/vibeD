...
- **Per-app Service routing.** Caddy proxies to a per-app `Service` that follows the sandbox pod across restarts — no more stale-pod-IP 502s. And when a pod is recreated, the controller re-injects source, so apps self-heal.
