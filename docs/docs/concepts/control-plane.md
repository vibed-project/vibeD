...
| **vibed-controller** | `:8081` (`--metrics-bind-address`) | `:8082` (`--health-probe-bind-address`); `healthz`, `readyz` | none (watches the API server) |
...
| **vibed-agent** | none | `GET /healthz` on the control API | Control API on `:9000` (`VIBED_AGENT_CONTROL_ADDR`); user app on `:8080` (`VIBED_AGENT_APP_PORT`) |
| **vibed-egress-authz** | `--metrics-bind-address` (default `0` = disabled) | none | `GET /authz` on `:8090` (`--addr`) |
| **vibed-workerd-loader** | none | none | Control API on `:9200` (`--listen`) |
