...
A resolver that returns a **named** tenant (non-empty `ID`) but leaves `Namespace` empty is a bug, because falling back to the shared default namespace would silently mix that tenant's apps in with everyone else's. The deploy path rejects it:
