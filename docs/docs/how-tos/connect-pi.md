...
`lifecycle` controls when the adapter opens the connection: `lazy` (default — on first tool call), `eager` (at startup), `keep-alive`, or `lazy-keep-alive`. `lazy` is fine for vibeD; deploys are infrequent and the first call adds only a few hundred milliseconds.
