...
3. Authentication: custom connectors authenticate with **OAuth**, not a static header. Run vibeD in [OIDC mode](../configuration/authentication.md#oidc-mode), which publishes `/.well-known/oauth-protected-resource` so the connector can discover your identity provider, and, if your IdP needs it, enter the client ID/secret under **Advanced settings**.
