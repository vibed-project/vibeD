...
// Out-of-tree providers (e.g. the enterprise PostgreSQL store and SAML auth
// mode) register themselves in the internal/store and internal/auth registries
// at init() time. The config validator must then accept their store.backend /
// auth.mode values — but config is a leaf package that must NOT import those
// registries (internal/auth already imports config, so importing it back would
// create a cycle). The server therefore injects small lister functions here
// before Load runs. When they are unset — bare config tests, or tools that
// never boot a server — only the built-in names are valid, so validation stays
// strict by default.
var (
...
// list for a validation error message, so an operator running the enterprise
// binary sees that e.g. "postgres" is in fact available. The empty mode (the
